...
# emotions.py (заглушка)
EMOTION_LABELS = ['sad', 'angry', 'neutral', 'positive']

def analyze_emotions(performance_info, audio_path):
    print(f"--- [Emotion STUB] Using mock emotion data ---")
    result = []
    for item in performance_info:
        speaker, text, start, end = item
        # Присваиваем эмоции по циклу: neutral → positive → neutral
        idx = len(result) % 4
        emotion = EMOTION_LABELS[idx]
        score = 0.6 + (idx * 0.1)  # от 0.6 до 0.9
        result.append([speaker, text, start, end, emotion, score])
    return result